...
describe("Duration value object class tests", () => {
    ...
    test.each(["1", "30", "50"])("Should create an value object for %s", (value) => {
    ...
    test.each(["", null, undefined, "a", "-5", "0", "1.5", "2,6"])("Should throw error for %s", (value) => {

        expect.assertions(1);
        expect(() => Duration.of(value)).toThrowError(ErrorMessage.INVALID_PARAM("duration value"));
    })

    test.each(["1", "21", "50"])("method equals() of a Duration object %s should return true for %s", (value) => {
        expect.assertions(1);
