...
describe("Update barber use case tests", () => {
    ...
    const oldProps = createFakeBarberProps();
    const newProps = createFakeBarberProps();

    test("Should update a Barber", async () => {

        const { repository, repositorySpy } = setupRepository<Barber>("update");
        const sut = new UpdateBarberUseCase(repository);

        const original = Barber.create(oldProps);
        const expUpdated = Barber.create(newProps, original.id);
        ...
        expect.assertions(3);
        expect(await repository.findById(original.id)).toMatchObject({ ...Barber });
    ...
    describe('Should throw Error: ', () => {
        ...
        test.each(["1", null, undefined, ""])("when trying to update a inexistent barber", async (id) => {
