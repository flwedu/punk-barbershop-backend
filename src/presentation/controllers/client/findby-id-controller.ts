...
import { createBadRequestErrorResponse, createOkResponse } from "../../http/response-entity-functions";
import ResponseEntity from "../../http/ResponseEntity";
import Controller from "../Controller";

export default class FindByIdController<T extends Entity> implements Controller {
    ...
    async handle(data: any): Promise<ResponseEntity<T>> {
        ...
        try {
            const response = await new FindByIdUseCase(this.repository).execute(data);
